...
export async function uploadFileToChannel(graphService, teamId, channelId, filePath, customFileName) {
    ...
    const channelFolderId = filesFolder.id;
...
export async function uploadFileToChat(graphService, filePath, customFileName) {
    ...
    if (uploadResult.id) {
        try {
            ...
            console.error(`[teams-mcp] createLink (organization) failed for item ${uploadResult.id}:`, orgErr instanceof Error ? orgErr.message : orgErr);
            try {
                const linkResponse = (await client
                    .api(`/drives/${driveId}/items/${uploadResult.id}/createLink`)
                ...
                console.error(`[teams-mcp] createLink (users) also failed for item ${uploadResult.id}:`, usersErr instanceof Error ? usersErr.message : usersErr);
            }
        }
    }
    return {
