 ...
 */
export function formatSearchHits(hits, contentFormat = "markdown") {
    return hits.map((hit) => ({
        id: hit.resource.id,
        summary: hit.summary,
        rank: hit.rank,
...
export function registerSearchTools(server, graphService, _readOnly) {
    ...
    server.registerTool("get_my_mentions", {
        title: "Get My Mentions",
        description: "Find recent messages where the current user was @mentioned across all Teams channels and chats.",
        inputSchema: {
            hours: z
                .number()
                .min(1)
