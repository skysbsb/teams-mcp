...
export function registerTeamsTools(server, graphService, readOnly) {
    ...
    server.registerTool("list_teams", {
        title: "List Teams",
        description: "List all Microsoft Teams that the current user is a member of. Returns team names, descriptions, and IDs.",
        inputSchema: {},
        annotations: {
            readOnlyHint: true,
    ...
    server.registerTool("list_channels", {
        title: "List Channels",
        description: "List all channels in a specific Microsoft Team. Returns channel names, descriptions, types, and IDs for the specified team.",
        inputSchema: {
    ...
    server.registerTool("get_channel_messages", {
        title: "Get Channel Messages",
        description: "Retrieve recent messages from a specific channel in a Microsoft Team. Returns message content, sender information, and timestamps.",
        inputSchema: {
            teamId: z.string().describe("Team ID"),
    ...
    if (!readOnly)
        server.registerTool("send_channel_message", {
            ...
            description: "Send a message to a specific channel in a Microsoft Team. Supports text and markdown formatting, mentions, and importance levels.",
            inputSchema: {
                teamId: z.string().describe("Team ID"),
                channelId: z.string().describe("Channel ID"),
    ...
    server.registerTool("get_channel_message_replies", {
        title: "Get Channel Message Replies",
        description: "Get all replies to a specific message in a channel. Returns reply content, sender information, and timestamps.",
        inputSchema: {
            teamId: z.string().describe("Team ID"),
            channelId: z.string().describe("Channel ID"),
        ...
        try {
            ...
            const repliesList = response.value.map((reply) => ({
                id: reply.id,
                content: formatMessageContent(reply.body?.content, effectiveContentFormat, reply.mentions),
    ...
    if (!readOnly)
        server.registerTool("reply_to_channel_message", {
            title: "Reply to Channel Message",
            description: "Reply to a specific message in a channel. Supports text and markdown formatting, mentions, and importance levels.",
            inputSchema: {
                teamId: z.string().describe("Team ID"),
                channelId: z.string().describe("Channel ID"),
    ...
    server.registerTool("list_team_members", {
        ...
        description: "List all members of a specific Microsoft Team. Returns member names, email addresses, roles, and IDs.",
        inputSchema: {
            teamId: z.string().describe("Team ID"),
        },
        annotations: {
    ...
    server.registerTool("search_users_for_mentions", {
        title: "Search Users for Mentions",
        description: "Search for users to mention in messages. Returns users with their display names, email addresses, and mention IDs.",
        inputSchema: {
    ...
    server.registerTool("download_message_hosted_content", {
        title: "Download Message Hosted Content",
        description: "Download hosted content (such as images) from a Teams channel message. Returns the content as base64 encoded data along with metadata. Use this to retrieve images or other inline content embedded in messages.",
        inputSchema: {
            teamId: z.string().describe("Team ID"),
            channelId: z.string().describe("Channel ID"),
    ...
    if (!readOnly)
        server.registerTool("delete_channel_message", {
            ...
            description: "Soft delete a message in a channel. Only the message sender can delete their own messages. The message will be marked as deleted.",
            inputSchema: {
                teamId: z.string().describe("Team ID"),
                channelId: z.string().describe("Channel ID"),
    ...
    if (!readOnly)
        server.registerTool("update_channel_message", {
            ...
            description: "Update (edit) a message in a channel that was previously sent. Only the message sender can update their own messages.",
            inputSchema: {
                teamId: z.string().describe("Team ID"),
                channelId: z.string().describe("Channel ID"),
    ...
    if (!readOnly)
        server.registerTool("set_channel_message_reaction", {
            title: "Set Channel Message Reaction",
            description: "Add a reaction to a message in a Teams channel. Supports Unicode emoji characters and named reactions (like, angry, sad, laugh, heart, surprised). Can also react to replies.",
            inputSchema: {
                teamId: z.string().describe("Team ID"),
                channelId: z.string().describe("Channel ID"),
    ...
    if (!readOnly)
        server.registerTool("unset_channel_message_reaction", {
            title: "Unset Channel Message Reaction",
            description: "Remove a reaction from a message in a Teams channel. Can also remove reactions from replies.",
            inputSchema: {
                teamId: z.string().describe("Team ID"),
                channelId: z.string().describe("Channel ID"),
    ...
    if (!readOnly)
        server.registerTool("send_file_to_channel", {
            title: "Send File to Channel",
            description: "Upload a local file and send it as a message to a Teams channel. Supports any file type (PDF, DOCX, ZIP, images, etc.). The file is uploaded to the channel's SharePoint folder and sent as a reference attachment. If messageId is provided, the file is sent as a reply to that message (thread).",
            inputSchema: {
                teamId: z.string().describe("Team ID"),
                channelId: z.string().describe("Channel ID"),
